			...
			for ($i=0; $i < $cnt; $i++) {
				...
				if ( $s_month < 0 ) {        //  Сб./Вс. перед/после праздника или Праздник в Сб./Вс. перед/после даты
					...
					if ( ( $f_e + 7 == $ep_e ) && ( $s_month == - 1 && $s_date == 0 && $f_month == 1 && $f_date == 6 && ( $type == 204 || $type == 207 ) ) ) {
						...
						if ( $s_month == - 1) {		 
							if ($we == 0 && $s_date%7 == 0 && $s_date <= 0) {
								$s_date -= 7;							// Если Праздник в Вс., то Вс. перед праздником сдвигается на -7 дней,
							}
							elseif  ( $we == $s_date ) $name = "";		// Если Сб. приходится на самый день праздника,	то не отмечается
						}                                                
...
function ortcal_shift204 ($d, $date, $year) {				
	...
	$we = date( 'w', mktime (0, 0, 0, 12, 25+$dd, $year)); // День недели Рождества Христова
	if (!$we) $we = 7;
	...
	$ff_e = (date('U', mktime(0, 0, 0, 12, 25, $year))-$easter)/(DAY_IN_SECONDS)-(7 + $we);	// Кол-во дней до Недели Праотцов от Пасхи
		
	if ($d == 252) $d = $ff_e;									// Неделя 29-я
	else if ($d == $ff_e) {
		$d = 252;								// Неделя Праотцов
	}	
...
function ortcal_shift207 ($d, $date, $year) {						
	...
	// поэтому они меняются местами с соответствующими рядовым чтениями той Недели,
	// на которую придется в данном году Неделя свв. праотец.
	$ff_e = (date('U', mktime(0, 0, 0, 12, 25, $year)) - $easter)/DAY_IN_SECONDS - (7 + $we) - ($er_e - 168); // Кол-во дней до Недели Праотцов от Пасхи 
	if ( $d == 245 ) {										// Неделя 28-я
		$d = $ff_e;
	}                                    
	else if ( $d == $ff_e ) {
//		echo $year." Е: Праотцев ".($ff_e/7-7)." Воздвижение ".($er_e/7-7)." Отступка ".($er_e - 168)."<br>";
		$d = 245;
	}
